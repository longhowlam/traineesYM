...
Organiseer altijd je werk R in projecten. Een project kan bestaan uit verschillende scripts, notebooks, data en output, deze wil je handig in 1 folder hebben. In RStudio kan je heel handig met verschillende R projecten werken. Via de GUI kun je nieuwe projecten aanmaken en beheren: File > New Project...
...
In de tijd voor dplyr kon je in R kolommen in een data frame kon maken of wijzigen met de `$` operator.
...
Maar met `mutate` in het dplyr package kan je dit veel eleganter doen
...
Je kan ook zelf gemaakte functies gebruiken in de functie `mutate`.
...
## in de development versie van tidyr heb je vernieuwde functie
tidyr::pivot_long(stocks, -time, names_to = "stock", values_to = "price")
...
## en ook in de tidydevelopment versie heb je 
pivot_wide()
```


Zie ook https://tidyr.tidyverse.org/dev/articles/pivot.html
...
In R kun je factor variabelen (categorische variabelen, zoals bijvoorbeeld "geslacht" met twee 'levels; man en vrouw) bewerken met functies uit het `forcats` package. We geven een aantal voorbeelden op random gegenereerde data
...
Tijdstippen en datums met tijdstippen kunnen in R ook worden weergegven met een speciale data type `POSIXct`.
...
Als je in R twee datums/times van elkaar aftrekt krijg je in R een zogenaamde `difftime` data type, die een interval of duratie weergeeft.
...
Ja kan intervallen of durations ook zelf maken en optellen bij een datum.
...
Het mooie van `dplyr` is dat je het niet alleen kan gebruiken op data frames in R. Je kan het ook gebruiken op externe data in databases en, ook data in Spark. We geven hier eerst het Spark voorbeeld met het package `sparklyr`, en dan een MySQL voorbeeldje.
...
Met de functie `geom_map` kun je kaarten maken. Als je de kaart van Nederland wil maken zul je die eerst ergens vandaan moeten halen. Vanuit het `raster` package is er een handige functie `getData` om dit te doen. Deze moeten we nog door gSimplify en fortify halen voordat ze in ggplot gebruikt kunnen worden
...
Er is recentelijk een makkelijkere manier met het package `ggmap`. Je kan hiermee kaartjes van Google maps met de functie `get_googlemap` downloaden en als achtergrond kaart gebruiken voor plots. Helaas is dit niet meer gratis en moet je je registreren en een API key opvragen!!!  
...
Je kan gelukkig ook de gratis openstreetmaps of stamen maps gebruiken. Hier onder een simpel voorbeeldje, plot de restaurants in Amsterdam op een kaartje.
...
## De functie get_map gebruiken om een kaart te downloaden
## de functie getbb kan je gebruiken om een bounding box van een plaats te krijgen
