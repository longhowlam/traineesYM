...
Als een input variable niet linear is m.b.t. de target kan je deze niet-lineariteit modelleren met buckets (linear constante stukken). Een alternatief is  regression splines, aan elkaar geplakte polynomen.
...
## Verloop zien van een binaire target

Voor een categorische variable kan je de relatie met een binaire target zien mbv group by statements (en eventueel barcharts)

```{r}
### maak eerst een 0/1 target kolom aan, 0 is N (niet overleefd), 1 is Y (wel overleefd)
myTitan$Target = as.numeric(myTitan$Survived) - 1

myTitan %>% group_by(Sex) %>%  summarise(survival = mean(Target))

```

Hoe kan je een idee krijgen van de relatie tussen een binaire target en een numerieke variabele? Gebruik smoothing plots.

```{r, eval = FALSE}
library(ggplot2)

### zonder smoothing, je ziet haast niks
ggplot(myTitan, aes(Age, Target)) + geom_point()

### met smoothing, de curve laat de waargenomen survival rate zien per leeftijd
ggplot(myTitan, aes(Age, Target)) + geom_point() + geom_smooth()
```
...
```{r, eval = FALSE}
out.glm = glm(Survived ~ Sex + Age + Pclass + Fare,  data = TTrain , family = binomial)
summary(out.glm)
```
...
```{r, eval = FALSE}
library(rpart)
tree.out = rpart(Survived ~ Sex + Age + Pclass + Fare, data = TTrain)
...
Met de library `xgboost` kan je in R extreme gradient boosting modellen fitten. De aanroep is anders dan wat we tot nu toe gezien hebben. De `xgboost` functie moet een matrix met input variabelen worden meegegeven. Een handige optie in xgboost is de zogenaamde `monotone_constraints` waar je kan opgeven dat een variabele monotoon dalend of stijgend moet zijn.

```{r, eval = FALSE}
library(Matrix)
Titan_Inputmatrix = sparse.model.matrix(  ~ Sex + Age + Pclass + Fare, data = TTrain)
...
Laten we onze huizen voorbeeld er weer bij pakken
...
```{r, eval = FALSE}
Titan_Testmatrix = sparse.model.matrix(  ~ Sex + Age +Pclass + Fare, data = TTest)
...
Als je een tree of ensemble van trees hebt getrained kan je een idee krijgen welke variabelen in het model belangrijk zijn geweest in het trainings proces. Laten we voor de tree modellen die we hierboven getraind hebben de variable importance zien. De absolute waarde van de variable importance plots zijn minder belangrijk dan de relatieve waarden (vergelijking tussen de variabelen).
...
Om de transacties die we net hebben ingelezen te genruiken in het `arules` package moeten we dus deze eerst in een `transaction` object zetten zoals in  bovenstaande code chunck is gebeurd. Nu je de boodschappen als transaction object hebt kan je er market basket rules op los laten met behulp van het a-priori algoritme.
...
Als je heel veel teksten / documenten hebt wil je vaak weten wat voor onderwerpen er in die documenten in hoofdlijnen zitten. Je kan dit doen met topic modeling, een vaak gebruikt algoritme is Latent Dirichlet Allocation (LDA) in het `text2vec` package.
...
Om het een en ander te illustreren heb ik wat NOS nieuwsberichten gescraped en bekijken we wat er voor topics te vinden zijn. 


```{r, eval = FALSE}
# packages nodig voor deze analyse
library(text2vec)
library(dplyr)
library(stringr)
library(wordcloud)
...
# lees data in
NOS = readRDS("data/NOS_Nieuwsberichten.RDs")
# ondubbel op artikel....
NOS = distinct(NOS, titel, .keep_all = TRUE  )

# plaatje aantal artikelen per dag
NOS %>% group_by(Datum) %>% summarise(n=n()) %>% 
  ggplot(aes(Datum,n)) + geom_line()

```

Eerst wat preparatie van de teksten, zoals niet alpha num characters en stopwoorden weghalen.
...
## gebruik bovenstaande prep functie om art te bewerken
NOS = NOS %>% 
  mutate(
    id = 1:n(),
    art = prep_fun(art),
    NWoorden = str_count(art, "\\w+")
  )

## plaatje van aantal woorden en tabel met talen
ggplot(NOS, aes(NWoorden)) + geom_histogram(bins=70, col="black")
```


Filter te korte en te lang artikelen er uit
...
We gaan nu de nieuwsberichten tokenizen en maken een vocabulaire zonder stopwoorden. 
...
Er zijn twee packages om makkelijk vectoren van stopwoorden te maken De packages `stopwords` en `tidystopwords`. Dit laatste package is iets uitgebreider.
...
stopwoorden2 = generate_stoplist(lang_name = "Dutch")
```


Maak nu een vocabulaire met de functie `create_vocabulaire` waar je ook ngrams kunt opgeven. Om niet te veel termen te genereren bekijk ik hier alleen 1 en 2 grams. Dus losse woorden en termen met twee aaneengesloten woorden.
...
De vocabulaire bestaat uit (te) veel woorden, we kunnen gaan prunen. Haal te weinig en ook te veel voorkomende woorden weg.
...
pruned_vocab = prune_vocabulary(
  ...
  doc_proportion_min = 0.0025
)
pruned_vocab
```

Maak een wordcloud voor eerste inzchten.
...
pruned_vocab %>% 
  ...
  ggplot(aes(log(rank),log2(term_count))) + geom_point()

```


And now perform topic modeling. Maak eerst een zogenaamde document term matrix aan. In deze matrix is het aantal rijen het aantal documenten / nieuwsberichten en het aantal kolommen is het aantal termen.
...
resultaat in de vorm van tabel en grafiek
...
NOS$topic = apply(doc_topic_distr, 1, which.max)
NOS %>% group_by(topic) %>% summarise(n=n())

####### visualisatie ##################

## heel makkelijk een topic te spotten met engelse texten 
lda_model$plot()
